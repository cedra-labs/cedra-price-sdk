...
1. Run `cedra move compile` to compile the script
2. Run the following to simulate the transaction and create a gas profile:
   ```
   cedra move run-script --compiled-script-path build/Profiler/bytecode_scripts/main.mv --args hex:<update_data_payload_in_hex> --profile-gas
   ```
