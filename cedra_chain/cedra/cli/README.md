...
Install cedra cli with the same version specified in the ci workflows.

All the commands which submit transactions require an environment variable `CEDRA_PRIVATE_KEY` for the private key to be set.
...
capability. You can read more about it [here](https://github.com/wormhole-foundation/wormhole/blob/5255e933d68629f0643207b0f9d3fa797af5cbf7/cedra/deployer/sources/deployer.move).
...
The following steps are needed to upgrade our cedra contracts:

- Generate the hash for the new contract build
- Create a governance proposal, proposing the cedra package to be upgraded to this specific hash
...
let payload =
  DefaultStore.chains.cedra_testnet.generateGovernanceUpgradePayload(
...
let contract =
  DefaultStore.contracts
    .cedra_testnet_0x7e783b349d3e89cf5931af376ebeadbfab855b3fa239b7ada8f5a92fbea6b387;
await contract.executeGovernanceInstruction(
  "private-key-of-account-incedra",
  vaa,
);
```
