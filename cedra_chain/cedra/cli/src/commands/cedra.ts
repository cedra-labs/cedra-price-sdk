...
import { CedraAccount, CedraClient, BCS, TxnBuilderTypes } from "cedra";
import fs from "fs";
import sha3 from "js-sha3";
import { CedraChain } from "@pythnetwork/contract-manager/core/chains";
...
const NETWORK_CHOICES = Object.entries(DefaultStore.chains)
  .filter(([_, config]) => {
    return config instanceof CedraChain;
  })
  .map(([chain, _]) => {
    return chain;
  });
...
export const builder: (args: Argv<any>) => Argv<any> = (yargs) =>
  yargs
    ...
    .command(
      ...
      async (argv) => {
        const endpoint = (DefaultStore.chains[argv.network] as CedraChain)
          .rpcUrl;
...
function getSender() {
  const key = process.env["CEDRA_PRIVATE_KEY"];
  if (key === undefined) {
    throw new Error(
      `Please set the CEDRA_PRIVATE_KEY environment variable to the private key of the sender in hex format`,
    );
  }
  return new CedraAccount(new Uint8Array(Buffer.from(key, "hex")));
}
...
async function executeTransaction(
  ...
  const endpoint = (DefaultStore.chains[network] as CedraChain).rpcUrl;
  const client = new CedraClient(endpoint);
  const sender = getSender();
  console.log(
...
function buildPackage(dir: string, addrs?: string): Package {
  ...
  if (cedra.status !== 0) {
    console.error(cedra.stderr.toString("utf8"));
    console.error(cedra.stdout.toString("utf8"));
    process.exit(1);
  }

  const result: any = JSON.parse(cedra.stdout.toString("utf8"));
  const buildDirs = fs
