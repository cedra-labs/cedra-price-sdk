module oracle::batch_price_attestation {
    ...
    fun deserialize_price_info(cur: &mut Cursor<u8>): PriceInfo {
        ...
        let current_price = oracle::price::new(price, conf, expo, publish_time);
        if (status != price_status::new_trading()) {
            current_price = oracle::price::new(prev_price, prev_conf, expo, prev_publish_time);
        };
        ...
        price_info::new(
            ...
            price_feed::new(
                price_identifier,
                current_price,
                oracle::price::new(ema_price, ema_conf, expo, ema_timestamp),
            )
        )
    }

    #[test]
    #[expected_failure(abort_code = 65560, location = oracle::batch_price_attestation)]
