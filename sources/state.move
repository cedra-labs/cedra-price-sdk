module oracle::state {
    ...
    use oracle::contract_upgrade_hash::Hash;
    use oracle::data_source::DataSource;
    use oracle::price_info::PriceInfo;
    use std::table::{Self, Table};
    use oracle::set::{Self, Set};
    use std::vector;
    use oracle::error;
    use std::account;
    ...
    friend oracle::contract_upgrade;
    friend oracle::set_governance_data_source;
    friend oracle::set_update_fee;
    friend oracle::set_stale_price_threshold;
    friend oracle::set_data_sources;

    #[test_only]
    friend oracle::governance_test;
    ...
    /// the checked `oracle::get_price` method. This ensures that the price
    /// is recent enough.
    ...
    public(friend) fun init(
        ...
        signer_capability: account::SignerCapability) {
            move_to(oracle, StalePriceThreshold{
                threshold_secs: stale_price_threshold,
            });
            move_to(oracle, BaseUpdateFee{
                fee: update_fee,
            });
            ...
            move_to(oracle, DataSources{
                sources,
            });
            move_to(oracle, GovernanceDataSource{
                source: governance_data_source,
            });
            move_to(oracle, LastExecutedGovernanceSequence{
                sequence: 0,
            });
            move_to(oracle, SignerCapability{
                signer_capability: signer_capability,
            });
            move_to(oracle, LatestPriceInfo{
    ...
    public fun get_last_executed_governance_sequence(): u64 acquires LastExecutedGovernanceSequence {
        let last_executed_governance_sequence = borrow_global<LastExecutedGovernanceSequence>(@oracle);
    ...
    public fun price_info_cached(price_identifier: PriceIdentifier): bool acquires LatestPriceInfo {
        let latest_price_info = borrow_global<LatestPriceInfo>(@oracle);
    ...
    public fun get_contract_upgrade_authorized_hash(): Hash acquires ContractUpgradeAuthorized {
        assert!(exists<ContractUpgradeAuthorized>(@oracle), error::unauthorized_upgrade());
        let ContractUpgradeAuthorized { hash } = move_from<ContractUpgradeAuthorized>(@oracle);
        hash
    }

    // Setters
    public(friend) fun set_data_sources(new_sources: vector<DataSource>) acquires DataSources {
        let sources = &mut borrow_global_mut<DataSources>(@oracle).sources;
        set::empty(sources);
    ...
    public(friend) fun set_latest_price_info(price_identifier: PriceIdentifier, price_info: PriceInfo) acquires LatestPriceInfo {
        let latest_price_info = borrow_global_mut<LatestPriceInfo>(@oracle);
    ...
    public(friend) fun set_last_executed_governance_sequence(sequence: u64) acquires LastExecutedGovernanceSequence {
        let last_executed_governance_sequence = borrow_global_mut<LastExecutedGovernanceSequence>(@oracle);
    ...
    public(friend) fun set_contract_upgrade_authorized_hash(hash: Hash) acquires ContractUpgradeAuthorized, SignerCapability {
        if (exists<ContractUpgradeAuthorized>(@oracle)) {
